...
В `README.md` и `installer/quickdeck.iss` в ссылках стоит `OWNER`. Заменить на свой логин:
...
1. Скачать собранный `QuickDeck-Setup-1.0.0.exe` из релиза и отправить его на перепроверку
   в [Kaspersky OpenTIP](https://opentip.kaspersky.com) и
   [Microsoft](https://www.microsoft.com/en-us/wdsi/filesubmission) — это снимает блокировки
   у всех пользователей сразу, а не на одной машине.
2. Когда наберётся пара выпусков, подать заявку на бесплатную подпись кода в
   [SignPath Foundation](https://signpath.io/solutions/open-source-community): проект
   открытый, под MIT, собирается на CI из исходников — их условия это покрывает.
   Подпись убирает причину придирок целиком.
